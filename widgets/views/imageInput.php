...
<div class="file-input <?= (empty($value) ? 'no-file' : 'has-file') ?>" id="<?= $widget->id ?>">
	...
	<div class="btn-toolbar actions-bar">
		...
		<div class="btn-group">
			<?php if ($widget->upload) { ?>
				<span class="btn btn-success choose-button">
					...
					<span class="no-file"><?php echo Yii::t('maddoger/admin', 'Upload…'); ?></span>
					<span class="has-file"><?php echo Yii::t('maddoger/admin', 'Replace…'); ?></span>
			...
			<?php } else {  ?>
				<a href="javascript:void(0);" class="browse-server-button btn btn-success ">
					<span class="no-file"><?php echo Yii::t('maddoger/admin', 'Browse server'); ?></span>
					<span class="has-file"><?php echo Yii::t('maddoger/admin', 'Replace…'); ?></span>
				</a>
			<?php } ?>
			...
			<?php if ($widget->setUrl || ($widget->upload && $widget->browseServer)) : ?>
				<button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">
					...
					<span class="sr-only"><?php echo Yii::t('maddoger/admin', 'Toggle Dropdown'); ?></span>
				</button>
				<ul class="dropdown-menu" role="menu">
					<?php if ($widget->upload && $widget->browseServer) : ?>
						<li><a href="javascript:void(0);" class="browse-server-button"><?php echo Yii::t('maddoger/admin', 'Browse server'); ?></a></li>
					<?php endif; ?>
					<?php if ($widget->setUrl) : ?>
						<li><a href="javascript:void(0);" class="set-url-button"><?php echo Yii::t('maddoger/admin', 'Set URL'); ?></a></li>
					<?php endif; ?>
				</ul>
			<?php endif; ?>
		</div>
        ...
        <span class="btn btn-warning <?= (empty($value) ? 'disabled' : '') ?> clear-button">
			<i class="fa fa-trash-o"></i> <?php echo Yii::t('maddoger/admin', 'Remove'); ?>
		</span>

		 <button type="reset" class="btn btn-default disabled reset-button">
			<i class="fa fa-refresh"></i> <?php echo Yii::t('maddoger/admin', 'Reset'); ?>
		 </button>
	</div>
	<div class="overlay">
		...
		<div class="drop-hint">
                <span
					class="drop-hint-info"><?php echo Yii::t('maddoger/admin', 'Drop Image Here…'); ?></span>
		</div>
	</div>
</div>